...
{
	char coiso[4096],newmsgfinal[4056],newmsg[4096],msgfinal[4096],msgenviada[4096], msgreceb[4096], user1[4096],msg[4096]; // dois chars
	int i=0,j=0,ult;
	int p=0;
  	char *msg1;
  	const char *mensagem;
	...
	write(meusocket,msgfinal,strlen(msgfinal));
	...
	for(i=0; i < ult;i++)
  	...
  	for(j=i;j < strlen(newmsg);j++)
   	...
   	for(j=0;j < strlen(newmsg);j++)
   	{
   	 	if(newmsg[j] != ' ')
   	 	{
   	 		newmsgfinal[j] = newmsg[j];
   	 		
   	 	}
   	}
  	strcat(newmsgfinal,"\n");
  	
	mensagem = g_convert(newmsgfinal, -1, "UTF-8", "ISO-8859-1", &bytes_read, &bytes_written, &error);
